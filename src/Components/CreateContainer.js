      ...
      <div className='w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4'>
        {
          fields && (
            <motion.p initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className={`w-full p-2 rounded-lg text-center text-lg font-semibold ${alertStatus === "danger" ? "bg-red-400 text-red-800" : "bg-emerald-400 bg-emerald-800"}`
              }>
              {msg}
            </motion.p>
          )
        ...
        <div className='w-full py-2 border-b border-gray-300 flex items-center gap-2'>
          ...
          <input type="text" required value={title}
            ...
            placeholder='Give me a tittle...' className='w-full h-full text-lg bg-transparent outline-none border-none placeholder:text-gray-300 text-textColor' />
        ...
        <div className='w-full'>
          <select onChange={(e) => setCategory(e.target.value)} className="outline-none w-full text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
            ...
            {categories && categories.map(item => (
              <option key={item.id} className="text-base border-0 outline-none capitalize bg-while text-headingColor" value={item.urlParamName}>
                {item.name}
              </option>
            ))}
          </select>
          ...
          {isLoading ? <Loader /> : <>
            {!imageAsset ? <>
              <label className='w-full h-full flex flex-col items-center justify-center cursor-pointer'>
                ...
                <input type="file" name="uploadimage" accept='image/*' onChange={uploadImage} class="w-0 h-0" />
        ...
        <div className='w-ful flex flex-col md:flex-row items-center gap-3'>
          <div className='w-full py-2 border-b border-gray-300 flex items-center gap-2'>
